...
int main(int argc, char *argv[])
{
	...
	for(int i=0;i<read_num_threads;i++)
	{
		...
		*arg = read_num_threads + i;
		int ret = pthread_create(threads+count,NULL,Reader,(void*) arg);
		int ret2 = pthread_create(threads+count+1,NULL,Writer,(void*) arg);
		if(ret)
    {
        printf("Error - pthread_create() return code: %d\n",ret);
        exit(EXIT_FAILURE);
...
// _____________NEW ___________________________________________________
	// for(int i=0;i<write_num_threads;i++)
	// {
	// 	int *arg = (int *)malloc((sizeof(int)));
	// 	if (arg == NULL)
 ...
 //    }
	...
	for (int i = 0; i < 2*write_num_threads; i++)
